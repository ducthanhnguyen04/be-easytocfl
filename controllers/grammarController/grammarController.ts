...
class GrammarController {
  ...
  async createGrammar(req: Request, res: Response): Promise<Response> {
    try {
      const { grammar, structure, usage, definition, notes, note, lessonId } = req.body as CreateGrammarDto;
      if (!grammar || !structure || !usage || !definition || !lessonId) {
        return res.status(400).json({ message: 'All are required' });
      }
      const newGrammar = await grammarService.createGrammar({ grammar, structure, usage, definition, note: note || notes, lessonId });
  ...
  async updateGrammar(req: Request, res: Response): Promise<Response> {
    try {
      ...
      const updatedGrammar = await grammarService.updateGrammar(id, { grammar, structure, usage, definition, note: note || notes, lessonId });
